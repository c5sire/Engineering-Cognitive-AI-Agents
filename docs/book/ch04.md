...
This model represents whether a new conversational episode has begun (`is_new_episode`) and specifies which elements of the current context should be retained (`preserve_context`).
...
The `KnowledgeActionType` enumeration reflects the different outcomes of the agent's analysis, while the `StoreKnowledgeResult` contains additional context about what action was taken, the rationale, and any associated metadata.
